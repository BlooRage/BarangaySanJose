...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_invite') {
        ...
        if (!in_array($roleAccess, ['Personnel', 'SuperAdmin'], true)) {
            set_invite_flash('danger', 'Role must be Personnel or SuperAdmin.');
            redirect_self();
        }
        if ($positionAccess === '') {
                    ...
                    <div class="invite-section mb-3">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <div class="invite-help">Choose the role, then department, then the matching title or position.</div>
                            </div>
                            <div class="col-md-6" id="departmentGroup" style="display:none;">
                            ...
                            <div class="col-md-6" id="positionAccessGroup" style="display:none;">
                                <label class="form-label" for="positionAccessSelect">Title / Position <span class="invite-required">*</span></label>
                                <select class="form-select" name="position_access" id="positionAccessSelect" required disabled>
                            ...
                            <div class="col-md-6" id="areaNumberGroup" style="display:none;">
                                <label class="form-label">Area Designation <span class="invite-required" id="areaRequiredMark">*</span></label>
                                <select class="form-select" name="area_number" id="areaNumberSelect" disabled>
...
<script>
  (function () {
	    ...
	    const positionGroup = document.getElementById('positionAccessGroup');
	    ...
	    const setGroupVisible = function (groupEl, visible) {
	      if (!groupEl) return;
	    ...
	    const syncPersonnelPositionOptions = function (preferredPosition) {
	      const selectedDepartment = departmentSelect.value;
	      const positions = getPersonnelPositionsForDepartment(selectedDepartment);
	      refillOptions(positionSelect, positions, selectedDepartment ? (positions.length ? 'Select Title / Position' : 'No titles for selected department') : 'Select Department First');
	      if (preferredPosition && positions.includes(preferredPosition)) {
	        positionSelect.value = preferredPosition;
	      }
	      setGroupVisible(positionGroup, selectedDepartment !== '');
	      positionSelect.disabled = selectedDepartment === '' || positions.length === 0;
	    };
	    ...
	    const renderPersonnelAccessPreview = function () {
	      ...
	      if (!personnelProfiles.length) {
	        ...
	        previewMetaEl.textContent = 'Create the title and module access in Personnel Access Control before sending a personnel invite.';
	        previewModulesEl.innerHTML = '';
	        return;
	      }
	      if (!profile) {
	    ...
	    const syncRoleConditions = function (preferred) {
	      ...
	      if (role === 'SuperAdmin') {
	        setGroupVisible(departmentGroup, true);
	        setGroupVisible(positionGroup, true);
	        ...
	        departmentHelp.textContent = 'SuperAdmin invites are assigned to Office of the Barangay.';
	        ...
	        syncPersonnelPositionOptions(currentPosition);
	        departmentHelp.textContent = departments.length
	          ? 'Select the department first. Titles will load from its Personnel Access Control profiles.'
	          : 'Create a Personnel Access Control profile before sending personnel invites.';
	        if (positionHelp) positionHelp.textContent = departmentSelect.value
	          ? 'Titles come from Personnel Access Control and carry the module permissions shown below.'
	          : 'Select a department first to load its titles from Personnel Access Control.';
	      } else {
	        setGroupVisible(departmentGroup, false);
	        setGroupVisible(positionGroup, false);
	    ...
	    const syncAreaRequirement = function () {
	      ...
	      const showArea = needsArea || (role === 'Personnel' && position !== '');
	      setGroupVisible(areaGroup, showArea);
	      areaSelect.disabled = !showArea;
	    ...
	    roleSelect.addEventListener('change', function () {
	      ...
	      areaSelect.value = '';
	      syncRoleConditions({ department: '', position_access: '' });
	    });
