                        ...
                        <form method="post" action="<?= htmlspecialchars(appUrl('/PhpFiles/Admin-End/appointmentManagement.php'), ENT_QUOTES, 'UTF-8') ?>" id="appointmentReviewForm" class="tracker-profile-view">
                            ...
                            <div class="tracker-form-grid cols-3">
                                ...
                                <div class="tracker-form-field">
                                    ...
                                    <input class="form-control" type="time" name="confirmed_time" id="reviewConfirmedTime" min="09:00" max="17:00">
                                </div>
                            </div>
...
<script>
    (() => {
        ...
        function formatReviewTime(value) {
            const text = String(value || "").trim();
            if (!text) return "-";
            const parsed = new Date(`2000-01-01T${text}`);
            if (Number.isNaN(parsed.getTime())) {
                return text;
            }
            return new Intl.DateTimeFormat("en-US", {
        ...
        function isRescheduleChanged() {
            const currentDate = String(reviewConfirmedDate?.value || "").trim();
            const currentTime = String(reviewConfirmedTime?.value || "").trim();
            const originalDate = String(modal?.dataset.originalReviewDate || "").trim();
            const originalTime = String(modal?.dataset.originalReviewTime || "").trim();
            return currentDate !== "" && currentTime !== "" && (currentDate !== originalDate || currentTime !== originalTime);
        }

        function isWithinOfficeHours(value) {
            const time = String(value || "").trim();
            return time !== "" && time >= "09:00" && time <= "17:00";
        }

        function syncReviewActionStates() {
            const isPending = String(modal?.dataset.reviewStatusKey || "").trim() === "pending";
            const hasOfficeHoursError = String(reviewConfirmedTime?.value || "").trim() !== "" && !isWithinOfficeHours(reviewConfirmedTime?.value || "");
            reviewScheduleError?.classList.toggle("d-none", !isPending || !hasOfficeHoursError);
            reviewActionButtons.forEach((actionButton) => {
                const action = String(actionButton.dataset.reviewAction || "").trim();
                if (action === "reschedule_appointment") {
                    actionButton.disabled = !isPending || !isRescheduleChanged() || hasOfficeHoursError;
                    return;
                }
        ...
        reviewConfirmedDate?.addEventListener("input", syncReviewActionStates);
        reviewConfirmedTime?.addEventListener("input", syncReviewActionStates);
