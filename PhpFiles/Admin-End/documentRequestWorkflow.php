...
if ($action === 'barangay_id_template_config') {
    ...
    $frontPublicUrl = dra_public_asset_path($frontPublicPath);
    $backPublicUrl = dra_public_asset_path($backPublicPath);
    dr_respond_json(200, [
        'success' => true,
        'front_template_url' => $frontPublicUrl !== ''
            ? $frontPublicUrl . ($frontVersion !== '' ? '?v=' . rawurlencode($frontVersion) : '')
            : '',
        'back_template_url' => $backPublicUrl !== ''
            ? $backPublicUrl . ($backVersion !== '' ? '?v=' . rawurlencode($backVersion) : '')
            : '',
...
function dra_render_signature_image(object $pdf, string $publicPath, float $x, float $y, float $w, float $h, int $scalePercent = 160, float $offsetXPercent = 0.0, float $offsetYPercent = 0.0): void
{
    ...
    try {
        ...
        if (is_array($size) && (int)$size[0] > 0 && (int)$size[1] > 0) {
            ...
            $x += ($w - $renderWidth) / 2.0;
            $y += $h - $renderHeight;
            $x += $w * max(-50.0, min(50.0, $offsetXPercent)) / 100.0;
            $y += $h * max(-50.0, min(50.0, $offsetYPercent)) / 100.0;
            $w = $renderWidth;
            $h = $renderHeight;
        }
...
function dra_generate_issued_document(array $requestRow): ?string
{
    ...
    $punongSignatureScale = max(50, min(160, (int)($moduleSettings['punong']['signature_scale_percent'] ?? 100)));
    $punongSignatureOffsetX = max(-50.0, min(50.0, (float)($moduleSettings['punong']['signature_offset_x_percent'] ?? 0)));
    $punongSignatureOffsetY = max(-50.0, min(50.0, (float)($moduleSettings['punong']['signature_offset_y_percent'] ?? 0)));
            ...
            try {
                ...
                $fitMultiline = static function (
                    ...
                    while ($size >= $hardMinSize) {
                        $pdf->SetFont('Arial', $style, $size);
                        $lines = $preserveFullSizeWrap ? $fullSizeLines : $wrapTextToWidth($pdf, $text, $w);
                ...
                $renderLayoutField = static function (FPDF $pdf, array $field) use (
                    ...
                    if ($type === 'signatory') {
                        dra_render_signature_image($pdf, $punongSignaturePath, $x + 1.1, max(0.0, $y - 0.1), max(4.0, $w - 2.0), max(2.4, $h * 0.52), $punongSignatureScale, $punongSignatureOffsetX, $punongSignatureOffsetY);
                    ...
                    $fieldSource = trim((string)($field['source'] ?? ''));
                    if (in_array($fieldSource, ['cardFullAddress', 'cardEmergencyAddress'], true)) {
                        $fontSize *= 3.0;
                        $minFontSize *= 3.0;
                    }
    ...
    if ($isGeneralPermitClearance) {
        ...
        if (class_exists('\\setasign\\Fpdi\\Fpdi') && is_file($templatePath)) {
            try {
                ...
                $pdf->Rect(120.5, 190.5, max(80.0, $pageWidth - 120.5), 24.5, 'F');
                dra_render_signature_image($pdf, $punongSignaturePath, 132.0, 185.8, 52.0, 9.2, $punongSignatureScale, $punongSignatureOffsetX, $punongSignatureOffsetY);
                $pdf->Line(126.2, 200.3, 194.6, 200.3);
                $writeFittedCell($pdf, 124.0, 201.5, 72.0, 5.6, $punongSignatoryName !== '' ? $punongSignatoryName : '-', 'B', 10.8, 8.4, 'C');
                $writeFittedCell($pdf, 124.0, 207.1, 72.0, 5.1, $punongSignatoryTitle !== '' ? $punongSignatoryTitle : '-', 'I', 10.0, 8.0, 'C');

                $pdf->Rect(120.5, 216.5, max(80.0, $pageWidth - 120.5), 24.5, 'F');
                dra_render_signature_image($pdf, $monitoringSignaturePath, 132.0, 211.8, 52.0, 9.2);
                $pdf->Line(126.2, 226.3, 194.6, 226.3);
                $writeFittedCell($pdf, 124.0, 227.5, 72.0, 5.6, $monitoringSignatoryName !== '' ? $monitoringSignatoryName : '-', 'B', 10.8, 8.4, 'C');
                $writeFittedCell($pdf, 124.0, 233.0, 72.0, 5.0, $monitoringSignatoryTitle !== '' ? $monitoringSignatoryTitle : '-', 'I', 9.8, 7.8, 'C');
    ...
    if ($isTricyclePermitClearance) {
        ...
        if (class_exists('\\setasign\\Fpdi\\Fpdi') && is_file($templatePath)) {
            try {
                ...
                dra_render_signature_image($pdf, $punongSignaturePath, 132.2, 197.8, 52.0, 9.2, $punongSignatureScale, $punongSignatureOffsetX, $punongSignatureOffsetY);
    ...
    if ($isBusinessPermitClearance) {
        ...
        if (class_exists('\\setasign\\Fpdi\\Fpdi') && is_file($templatePath)) {
            try {
                ...
                dra_render_signature_image($pdf, $punongSignaturePath, 126.0, 242.0, 52.0, 8.8, $punongSignatureScale, $punongSignatureOffsetX, $punongSignatureOffsetY);
    ...
    if ($isRelationshipJailVisit) {
        ...
        if (class_exists('\\setasign\\Fpdi\\Fpdi') && is_file($templatePath)) {
            try {
                ...
                if (is_file($qrDiskPath)) {
                    $pdf->Image($qrDiskPath, 186.0, 248.0, 20.0, 20.0);
                }
    ...
    if ($isSpecialCertificate) {
        ...
        if ($isFirstTimeJobSeeker) {
            ...
            $signX = 126.0;
            $signW = 46.0;
            dra_render_signature_image($pdf, $punongSignaturePath, $signX + 1.0, $signBaseY - 21.0, $signW - 2.0, 20.0, $punongSignatureScale, $punongSignatureOffsetX, $punongSignatureOffsetY);
            ...
            dra_render_signature_image($pdf, $punongSignaturePath, 124.0, $signBaseY - 27.0, 70.0, 26.0, $punongSignatureScale, $punongSignatureOffsetX, $punongSignatureOffsetY);
        ...
        if (is_file($qrDiskPath)) {
            $fixedQrSize = 20.0;
            $pdf->Image($qrDiskPath, 186.0, 248.0, $fixedQrSize, $fixedQrSize);
        }
        $writeIndentedParagraph(
    ...
    $pdf->SetY(250);
    dra_render_signature_image($pdf, $punongSignaturePath, 18.0, 225.0, 70.0, 24.0, $punongSignatureScale, $punongSignatureOffsetX, $punongSignatureOffsetY);
    ...
    try {
        ...
        for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
            ...
            if ($pageNo === $pageCount) {
                ...
                $y = max(4.0, $pageHeight - $qrSize - $margin - 4.0);
        ...
        for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
            ...
            if ($pageNo === $pageCount) {
                ...
                if ($qrReal !== null) {
                    ...
                    $qrY = max(0.0, $pageHeight - $qrSize - $qrMarginBottom - 4.0);
...
if ($action === 'submit_fee_change_request') {
    ...
    $feeCatalogSource = strtolower(trim((string)($_POST['fee_catalog_source'] ?? '')));
    ...
    $catalogStmt->bind_param('i', $feeTypeId);
    $catalogStmt->execute();
    ...
    $existingStmt->bind_param('i', $feeTypeId);
    ...
    if ($existingRow) {
        ...
        if (!$stmt) dr_respond_json(500, ['success' => false, 'message' => 'DB error.']);
        $stmt->bind_param('isddssi', $requestRowId, $feeName, $currentAmount, $proposedAmount, $notes, $userId, $feeTypeId);
        if (!$stmt->execute()) {
            $stmt->close();
