...
| R-47A | Finance Review | Finance rejects an OR number already used by another transaction | 1. Use an OR number already stored in an online or manual finance transaction. 2. Attempt to verify or record another payment with the same OR, including a casing variation. | Posting is rejected with an `OR number already exists` message and the request remains unchanged. | Critical | Not Run |
