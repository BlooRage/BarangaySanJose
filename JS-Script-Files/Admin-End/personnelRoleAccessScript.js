(function () {
  ...
  const renderTable = () => {
    ...
    if (!state.rows.length) {
      tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-4">No position permission profiles match the current filters.</td></tr>';
      return;
    }
  ...
  const openAccessModal = (profileId) => {
    const row = state.rowsRaw.find((entry) => String(entry.profile_id) === String(profileId));
    ...
    if (modalSourceEl) modalSourceEl.textContent = safe(row.access_source);
    if (modalModulesSummaryEl) modalModulesSummaryEl.textContent = `${row.permission_count || 0} granted - ${safe(row.permission_summary)}`;
    if (permissionSearchInput) permissionSearchInput.value = "";
    if (resetBtn) resetBtn.disabled = !row.has_saved_profile;
  ...
  const setLoadingState = (loading) => {
    ...
    if (resetBtn) resetBtn.disabled = loading || !Boolean(state.modal.row?.has_saved_profile);
  };
